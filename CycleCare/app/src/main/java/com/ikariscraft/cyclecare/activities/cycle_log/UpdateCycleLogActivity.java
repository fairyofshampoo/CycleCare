...
import com.ikariscraft.cyclecare.databinding.ActivityUpdateCycleLogBinding;
...
public class UpdateCycleLogActivity extends AppCompatActivity {
    private ActivityUpdateCycleLogBinding binding;
    private NewCycleLogViewModel viewModel;
    public static final String CYCLE_LOG_KEY = "cycle_log_key";
    public CycleLog cycleLog;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityUpdateCycleLogBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());

        viewModel = new ViewModelProvider(this).get(NewCycleLogViewModel.class);

        Bundle extras = getIntent().getExtras();
        cycleLog = extras.getParcelable("cycle_log_key");
    ...
    private void setUpOperationStatusListener() {
        viewModel.getUpdateCycleLogRequestStatus().observe(this, requestStatus -> {
            if (requestStatus == RequestStatus.DONE) {
                Toast.makeText(this, "Se ha actualizado exitosamente el registro del ciclo", Toast.LENGTH_SHORT).show();
                Intent intent = new Intent(this, PrincipalScreen.class);
                startActivity(intent);
            }

            if (requestStatus == RequestStatus.ERROR) {
                ProcessErrorCodes errorCode = viewModel.getUpdateCycleLogErrorCode().getValue();

                if(errorCode != null) {
                    showUpdateCycleLogError(errorCode);
                }
            }
        });
    }

    private void showUpdateCycleLogError(ProcessErrorCodes errorCode) {
        String message = "";
        switch (errorCode) {
            case NOT_FOUND_ERROR:
                message = getString(R.string.cycle_log_not_found);
                break;
    ...
    private void setUpButtons() {
        binding.btnSave.setOnClickListener(v -> {
            if(viewModel.getUpdateCycleLogRequestStatus().getValue() != RequestStatus.LOADING) {
                updateCycleLog();
            }
        });
    ...
    private void updateCycleLog() {
        SessionSingleton sessionSingleton = SessionSingleton.getInstance();
        String token = sessionSingleton.getToken();
        NewCycleLogBody newCycleLogBody = new NewCycleLogBody();
        newCycleLogBody.setCycleLogId(cycleLog.getCycleLogId());
        newCycleLogBody.setCreationDate(cycleLog.getCreationDate());
        ...
        viewModel.updateCycleLog(token, newCycleLogBody);
    }
    ...
    private void setCycleLogData() {
        ...
        if(cycleLog.getNote() != null){
            setNoteText(cycleLog.getNote());
        }

        if(cycleLog.getMenstrualFlowId() != null){
            setMenstrualFlow(cycleLog.getMenstrualFlowId());
        }
        if(cycleLog.getVaginalFlowId() != null){
            setVaginalFlow(cycleLog.getVaginalFlowId());
        }

        if(cycleLog.getSymptoms() != null) {
            setSymptoms(cycleLog.getSymptoms());
        }

        if (cycleLog.getMoods() != null) {
            setMoods(cycleLog.getMoods());
        }

        if(cycleLog.getMedications() != null) {
            setMedications(cycleLog.getMedications());
        }

        if(cycleLog.getPills() != null){
            setPills(cycleLog.getPills());
        }
        if(cycleLog.getBirthControl() != null) {
            setBirthControl(cycleLog.getBirthControl());
        }
    }

    private void setBirthControl(List<BirthControl> birthControl) {
        for (BirthControl control : birthControl) {
            switch (control.getBirthControlId()) {
                case 1:
                    binding.rbInserted.setChecked(true);
                    break;
                case 2:
    ...
    private void setPills(List<Pill> pills) {
        for (Pill pill : pills) {
            switch (pill.getPillId()) {
                case 1:
                    binding.pillTakenCheckBox.setChecked(true);
                    break;
                case 2:
                    binding.pillForgottenCheckBox.setChecked(true);
                    break;
                case 3:
                    binding.doubleDoseCheckBox.setChecked(true);
                    break;
                case 4:
                    binding.noDoseCheckBox.setChecked(true);
                    break;
                case 5:
                    binding.lateDoseCheckBox.setChecked(true);
                    break;
            }
        }
    }
    ...
    private void setMedications(List<Medication> medications) {
        for (Medication medication : medications) {
            switch (medication.getMedicationId()) {
                case 1:
                    binding.hormoneTherapyCheckBox.setChecked(true);
                    break;
                case 2:
                    binding.emergencyPillCheckBox.setChecked(true);
                    break;
                case 3:
                    binding.painkillersCheckBox.setChecked(true);
                    break;
                case 4:
                    binding.antidepressantsCheckBox.setChecked(true);
                    break;
                case 5:
                    binding.antibioticsCheckBox.setChecked(true);
                    break;
                case 6:
                    binding.antihistaminesCheckBox.setChecked(true);
                    break;
            }
        }
    }

    private void setMoods(List<Mood> moods) {
        for (Mood mood : moods) {
    ...
    private void setSymptoms(List<Symptom> symptoms) {
        for (Symptom symptom : symptoms) {
            switch (symptom.getSymptomId()) {
                case 1:
                    binding.abdominalPainCheckBox.setChecked(true);
                    break;
                case 2:
                    binding.breastTendernessCheckBox.setChecked(true);
                    break;
                case 3:
                    binding.acneCheckBox.setChecked(true);
                    break;
                case 4:
                    binding.bloatingCheckBox.setChecked(true);
                    break;
                case 5:
                    binding.fatigueCheckBox.setChecked(true);
                    break;
                case 6:
                    binding.headacheCheckBox.setChecked(true);
                    break;
                case 7:
                    binding.nauseaCheckBox.setChecked(true);
                    break;
                case 8:
                    binding.dizzinessCheckBox.setChecked(true);
                    break;
                case 9:
                    binding.cravingsCheckBox.setChecked(true);
                    break;
                case 10:
                    binding.constipationCheckBox.setChecked(true);
                    break;
                case 11:
                    binding.diarrheaCheckBox.setChecked(true);
                    break;
                case 12:
                    binding.vaginalItchingCheckBox.setChecked(true);
                    break;
                case 13:
                    binding.vulvaPainCheckBox.setChecked(true);
                    break;
            }
        }
    }
    ...
    private void setSleepHours(Integer sleepHours) {
        binding.sleepHours.setSelection(sleepHours);
    }
